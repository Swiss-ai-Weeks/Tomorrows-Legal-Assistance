...
with st.sidebar:
    ...
    # Audio toggle button
    audio_button_text = (
        "🔊 Music ON" if st.session_state.audio_enabled else "🔇 Music OFF"
    )
    ...
    if st.session_state.audio_enabled:
        ...
        # Simple instructions
        st.info(
            "Use the audio player above to play music manually while using the app."
        )
    else:
            ...
            for doc in st.session_state.source_documents:
                ...
                if "fallback" in doc_id:
                    ...
                    sr220_files = [
                        ...
                        for f in os.listdir(
                            os.path.join(
                                os.path.dirname(__file__), "..", "data", "swiss_law"
                            )
                        )
...
for message in st.session_state.messages:
    avatar_path = (
        "frontend/media/Gemini_Generated_Image_wl26p8wl26p8wl26_zoomin.png"
...
if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
    ...
    with st.chat_message(
        "assistant",
        avatar="frontend/media/Gemini_Generated_Image_wl26p8wl26p8wl26_zoomin.png",
    ):
        ...
        with st.spinner("Analyzing your situation..."):
            try:
                ...
                response = requests.post(
                    "http://localhost:8000/api/agent_with_tools", json=case_input
                )
