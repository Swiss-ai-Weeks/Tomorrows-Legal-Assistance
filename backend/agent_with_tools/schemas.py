...
class AgentOutput(BaseModel):
    ...
    likelihood_win: Optional[str] = Field(
        None, description="Likelihood of winning as percentage string (e.g., '60%')"
    )
    estimated_time: Optional[str] = Field(
        None, description="Time estimate (ISO 8601 duration or human string)"
    )
    estimated_cost: Optional[str] = Field(
        None, description="Cost estimate as string (e.g., '3500 CHF')"
    )
    explanation: Optional[str] = Field(
        None, description="Reasoning and explanation for the analysis"
    )
    source_documents: Optional[list[Doc]] = Field(
        None, description="Source documents used in the analysis"
    )
    final_answer: str = Field(
        "", description="The final answer that will be display to the user."
    )
...
class AgentState(BaseModel):
    ...
    explanation_parts: Optional[list[str]] = (
        None  # Collect explanation parts during analysis
    )
    source_documents: Optional[list[Doc]] = (
        None  # Collect source documents used during analysis
    )
...
class CostInputs(BaseModel):
    ...
    judges_count: Optional[int] = None
    hourly_rates: Optional[Dict[str, float]] = (
        None  # e.g., {"lawyer": 400, "paralegal": 150}
    )
